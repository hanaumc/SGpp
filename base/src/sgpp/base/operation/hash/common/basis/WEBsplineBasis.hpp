...
#ifndef WEBspline_BASE_HPP
#define WEBspline_BASE_HPP
...
/**
 * WEB-spline basis on Noboundary grids.
 */
template <class LT, class IT>
class WEBsplineBasis : public Basis<LT, IT> {
 public:
  /**
   * Default constructor.
   */
  WEBsplineBasis() : degree(0) {}

  /**
   * Constructor.
   *
   * @param degree    WEB-spline degree, must be odd
   *                  (if it's even, degree - 1 is used)
   */
  explicit WEBsplineBasis(size_t degree) : degree(degree) {
    if (degree < 1) {
      this->degree = 1;
    } else if (degree % 2 == 0) {
   ...
   * Destructor.
   */
  ~WEBsplineBasis() override {}

  /**
   * @param x     evaluation point
   * @param p     WEB-spline degree
   * @return      value of uniform WEB-spline
   *              (with knots \f$\{0, 1, ..., p+1\}\f$)
   */
  inline double uniformWEBspline(double x, size_t p) const {
    ...
    switch (p) {
      ...
      default:
        ...
        // ==> calculate WEB-spline value by Cox-de-Boor recursion
        const double pDbl = static_cast<double>(p);

        if ((x < 0.0) || (x >= pDbl + 1.0)) {
          return 0.0;
        } else {
          return (x / pDbl) * uniformWEBspline(x, p - 1) +
                 ((pDbl + 1.0 - x) / pDbl) * uniformWEBspline(x - 1.0, p - 1);
        }
    }
  }
   ...
   * @param x     evaluation point
   * @param p     WEB-spline degree
   * @return      value of derivative of uniform WEB-spline
   *              (with knots \f$\{0, 1, ..., p+1\}\f$)
   */
  inline double uniformWEBsplineDx(double x, size_t p) const {
    switch (p) {
      case 3:
        if ((x < 0.0) || (x >= 4.0)) {
          return 0.0;
      ...
      default:
        if ((x < 0.0) || (x >= static_cast<double>(p) + 1.0)) {
          ...
          return uniformWEBspline(x, p - 1) - uniformWEBspline(x - 1.0, p - 1);
        }
    }
  }
   ...
   * @param x     evaluation point
   * @param p     WEB-spline degree
   * @return      value of 2nd derivative of uniform WEB-spline
   *              (with knots \f$\{0, 1, ..., p+1\}\f$)
   */
  inline double uniformWEBsplineDxDx(double x, size_t p) const {
    switch (p) {
      case 3:
        if ((x < 0.0) || (x >= 4.0)) {
          return 0.0;
      ...
      default:
        if ((x < 0.0) || (x >= static_cast<double>(p) + 1.0)) {
          ...
          return uniformWEBspline(x, p - 2) - 2.0 * uniformWEBspline(x - 1.0, p - 2) +
                 uniformWEBspline(x - 2.0, p - 2);
        }
    }
  }
   ...
   * @param x     evaluation point
   * @return      value of WEB-spline basis function
   */
  inline double eval(LT l, IT i, double x) override {
    const double hInv = static_cast<double>(static_cast<IT>(1) << l);

    return uniformWEBspline(
   ...
   * @param i     index of basis function
   * @param x     evaluation point
   * @return      value of derivative of WEB-spline basis function
   */
  inline double evalDx(LT l, IT i, double x) {
    const double hInv = static_cast<double>(static_cast<IT>(1) << l);

    return hInv * uniformWEBsplineDx(x * hInv - static_cast<double>(i) +
   ...
   * @param i     index of basis function
   * @param x     evaluation point
   * @return      value of 2nd derivative of WEB-spline basis function
   */
  inline double evalDxDx(LT l, IT i, double x) {
    const double hInv = static_cast<double>(static_cast<IT>(1) << l);

    return hInv * hInv * uniformWEBsplineDxDx(x * hInv - static_cast<double>(i) +
                                                static_cast<double>(this->degree + 1) / 2.0,
                                            this->degree);
  }

  /**
   * @return      WEB-spline degree
   */
      ...
      switch (degree) {
        ...
        default:
          throw operation_exception(
              ...
              "WEBsplineBasis::getIntegral() only"
 ...
 protected:
  /// degree of the WEB-spline
  size_t degree;
};

// default type-def (unsigned int for level and index)
typedef WEBsplineBasis<unsigned int, unsigned int> SWEBsplineBase;

}  // namespace base
}  // namespace sgpp

#endif /* WEBspline_BASE_HPP */
