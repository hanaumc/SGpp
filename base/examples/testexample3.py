...
levelymax = 10
for levelx in range(2,6):
    for levely  in range(2,levelymax):
        start = time.time()
        print(levelx,levely)
        level_x = levelx
        level_y = levely   
        ...
        for p in range(len(I_all)):  
            ...
            for i in index_I_all: 
                ...
                extended_Bspline = extended_Bspline + ( bi + inner_sum)
                A_WEB[p,c] = weightfunction.ellipse(radius1, radius2, I_all[p])*extended_Bspline
                c=c+1          
        ...
        for i in range(len(I_all)):
            b_1[i] = function_1_ellipse(gp[int(index_I_all[i])])
        #print(b)
